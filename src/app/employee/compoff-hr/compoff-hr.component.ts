...
@Component({
  selector: 'app-compoff-hr',
  templateUrl: './compoff-hr.component.html',
  styleUrls: ['./compoff-hr.component.scss']
})
export class CompoffHRComponent implements OnInit {
  item: any;
  time: any;
  emp_code: any;
  req_id: any;
  ...
  startdate: any;
    ...
    this.apicall.FetchEmployeeList(-1,-1,this.empcode).subscribe((res) => {
      ...
      //alert(JSON.stringify(data))
       this.apicall.Filter_FetchCompoff_LeaveRequests_HR(data).subscribe((res) => {
    ...
    ActivereqID(reqid:any){
      ...
      onCompanySelected(selectedCompanyid: any) { 
        ...
        this.apicall.FetchDepartmentList(selectedCompanyid,this.selectedEmpCode).subscribe((res) => {
          this.deptdata=res;     
        }); 
        this.filter();       
       ...
       filter(){
        ...
        if( this.fromdate && this.todate && this.fromdate > this.todate){
          ...
          //alert(JSON.stringify(data))
           this.apicall.Filter_FetchCompoff_LeaveRequests_HR(data).subscribe((res) => {
            this.compoffdata=res;
            const maxPageFiltered = Math.ceil(this.compoffdata.length / this.itemsPerPage);  

          if (this.currentPage > maxPageFiltered) {
            this.currentPage = 1;     
          } 
            console.log(JSON.stringify(this.compoffdata)) 
...
const totalResults = this.compoffdata.filter((employee: any) => {
...
const filteredData = this.compoffdata.filter((employee: any) =>
