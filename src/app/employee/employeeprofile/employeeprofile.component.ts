 ...
 // employee_code= (<HTMLInputElement>document.getElementById("nonactiveEmp")).value;
  ...
  constructor(private session:LoginService,private apicall:ApiCallService,private route:Router,private fb: FormBuilder,private datePipe: DatePipe) { 
    ...
    this.empBasicForm = this.fb.group({
     ...
     // jobdercription: new FormControl (''),
      notperiod: new FormControl ('', [Validators.pattern("^[0-9]*$")]),
  ...
  ngOnInit(): void {
   ...
   this.fetchFamStatus();
    ...
    //     //this.empdocumentForm.controls['docname'].setValue(this.tableRows[0].KEY_VALUE);
        
    //     })  
    //     this.apicall.listmandocs().subscribe((res)=>{
    //       this.tableRows.controls=res;
    //     //  alert(JSON.stringify(this.tableRows.controls));
    //       //this.empdocumentForm.controls['docname'].setValue(this.tableRows[0].KEY_VALUE);
  ...
  displayempcode(company_id:any)
  {
      ...
      if(company_id=='MH')
    {
     ...
     // alert(cmbsel.value);
      ...
      this.comdtl=company_id;
       this.resdtl=company_id;
  }
  ...
  basicSubmit() 
  {
    ...
    else if(this.empBasicForm.valid)
    {
      ...
      this.apicall.addbasicdtls(this.empBasicForm.value).subscribe((res)=>{
...
extractFilename(filePath:any) {
  ...
  contactSubmit() 
  {
    ...
    else
    {
      (<HTMLInputElement>document.getElementById("openModalButtonForalert")).click();
      this.showModal = 1;
      this.success = "Please fill basic details...";
    }
  ...
  banksalarySubmit() 
  {
    ...
    else if(this.bankSalaryForm.valid)
    {
     ...
     // alert(JSON.stringify(this.bankSalaryForm.value));
        ...
        this.apicall.addbankdtls(this.bankSalaryForm.value).subscribe((res)=>{
...
else
{
  ...
  displaybankcode(bankid:any)
  {
   // alert(bankid)
    ...
    {
      this.bankSalaryForm.controls['CBIDcode'].setValue("");
      this.bankSalaryForm.controls['routing_code'].setValue("");
    }
  }
...
if(nonactiveEmp!="null")
{
  ...
  activateEmp()
  {
   ...
   if(nonactiveEmp!="null")
   {
    ...
    this.apicall.activateEmp(activeData).subscribe((res)=>{
      this.nonactiveEmp=res;
      // alert(JSON.stringify(this.nonactiveEmp) )
    if(res.Errorid==1)
    {
      //alert(JSON.stringify(res.Errormsg));
      //const originalString: string = this.nonactiveEmp[0].Errormsg;
     // alert(originalString);
      //const parts = originalString.split('^');
     // const secondPart = parts[1];
      (<HTMLInputElement>document.getElementById("openModalButtonForalert")).click();
      this.showModal = 1;
      this.success = "Successfully Activated..new code is: "+res.Errormsg;
      this.activatefolder(nonactiveEmp,res.Errormsg);
      this.fetchnonactiveEmp();
      this.Resetbasic();
    ...
    (<HTMLInputElement>document.getElementById("openModalButtonForalert")).click();
      this.showModal = 1;
      this.success = "Please select employee name";
   }

   

  }
  activatefolder(ecode:any,newcode:any)
  {
    this.apicall.UploadDirectory(ecode,newcode).subscribe((res)=>{
      const rs=res;
    });
  }
  deleteEmp()
  {
    ...
    this.apicall.deleteEmp(deleteData).subscribe((res)=>{
      this.activeEmp=res;
...
fetchbasicdetails() {
  ...
  const empcode = (<HTMLInputElement>document.getElementById("empcode")).value;

  
  let nonactEmp; // Declare nonactEmp outside the if-else block
  ...
  {
    nonactEmp = nonactiveEmp;
  }

  this.apicall.displaybasicdetails(nonactEmp).subscribe((res) => {
  ...
  this.apicall.FetchCompanyList(nonactiveEmp).subscribe((res)=>{
    ...
    this.apicall.FetchResouceList(nonactiveEmp).subscribe((res)=>{
     ...
     // alert(this.dispbasicdet[0].COMPANY_CODE);
      // if(this.fetchbankdtls[0].EMP_WPS_COMPANY==0 || this.fetchbankdtls[0].EMP_WPS_COMPANY== null )
      // {
        this.bankSalaryForm.controls['wpscompny'].setValue(this.dispbasicdet[0].COMPANY_CODE);
      // }
      // else
      // {
...
fetchbankdetails()
{
 ...
 const nonactiveEmp = (<HTMLInputElement>document.getElementById("nonactiveEmp")).value;
  ...
  this.apicall.fetchbankdtls(nonactEmp).subscribe((res)=>{
...
// alert(JSON.stringify(res));
  ...
  if( this.fetchbankdtls!="")
  {
    ...
    {
      this.bankSalaryForm.controls['salprocess'].setValue("");
    }
    else
    {
    ...
    {
      this.bankSalaryForm.controls['paymode'].setValue("");  
    }
    else
    {
    ...
    if(this.fetchbankdtls[0].EMP_WPS_COMPANY==0 || this.fetchbankdtls[0].EMP_WPS_COMPANY== null )
    {
      this.bankSalaryForm.controls['wpscompny'].setValue(this.fetchbankdtls[0].COMPANY_CODE);
    }
    else
    {
      this.bankSalaryForm.controls['wpscompny'].setValue(this.fetchbankdtls[0].EMP_WPS_COMPANY);
    ...
    {
      this.bankSalaryForm.controls['bankname'].setValue("null");
    }
    else
    {
...
Resetbasic()
{
  ...
  this.empBasicForm.get('fname')?.setErrors(null); 
   ...
   this.empBasicForm.controls['airdestination'].setValue("");
   this.empBasicForm.controls['aireligibility'].setValue("");
