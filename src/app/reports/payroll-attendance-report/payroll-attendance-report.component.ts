...
@Component({
  selector: 'app-payroll-attendance-report',
  templateUrl: './payroll-attendance-report.component.html',
  styleUrls: ['./payroll-attendance-report.component.scss']
})
export class PayrollAttendanceReportComponent implements OnInit {
  ...
  filter(){    
    this.apicall.FetchPayrollAttendanceDetails(this.month,this.year,this.companycode).subscribe((res)=>{
...
download_to_excel()
  { 
   let Excelname:any;
   this.apicall.ExportToExcel(this.attendancedata).subscribe((res)=>{
...
const totalResults = this.attendancedata.filter((policy: any) => {
...
const filteredData = this.attendancedata.filter((policy: any) =>
